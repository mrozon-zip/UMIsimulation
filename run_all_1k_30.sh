      ...
      for deviation in 0.01 0.05 0.1; do
        for mutation_rate in 0 0.001 0.002 0.005 0.01; do
          python3 main.py amplify --method --bridge \
            ...
            --deviation "$deviation" \
            --no_simulate \
            --output "bridge_mut_${mutation_rate}_Sr${s_radius}_dens${density}_SP${success_prob}_dev${deviation}"
        done
      done
    done
  done
      ...
      for deviation in 0.01 0.05 0.1; do
        for mutation_rate in 0 0.001 0.002 0.005 0.01; do
          python3 main.py amplify --method --polonies_amplification \
            ...
            --deviation "$deviation" \
            --no_simulate \
            --output "polonies_mut_${mutation_rate}_Sr${s_radius}_dens${density}_SP${success_prob}_dev${deviation}"
        done
      done
    done
  done
