...
def parse_args():
    ...
    parser.add_argument(
        "pattern",
        choices=["pcr", "polonies"],
        help="Substring to match in filenames (use 'pcr' or 'polonies')")
    return parser.parse_args()

def find_files(folder, pattern):
    """Return all file paths in `folder` whose names contain `pattern`."""
    return [
...
def plot_heatmap(matrix, born_values, active_values, output_filename):
    ...
    plt.title("Heatmap of Active vs. Birth Cycles (Locally Normalized)")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    output_filename = pattern + '_heatmap.png'
