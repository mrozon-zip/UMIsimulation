...
def main():
    ...
    # Identify files with extreme precision and accuracy metrics
    # Highest precision
    max_prec_idx = df['precision'].idxmax()
    file_max_prec = df.loc[max_prec_idx, 'file']
    print(f"Highest precision: {file_max_prec} ({df.loc[max_prec_idx, 'precision']})")
    # Lowest precision
    min_prec_idx = df['precision'].idxmin()
    file_min_prec = df.loc[min_prec_idx, 'file']
    print(f"Lowest precision: {file_min_prec} ({df.loc[min_prec_idx, 'precision']})")
    # Highest accuracy
    max_acc_idx = df['accuracy'].idxmax()
    file_max_acc = df.loc[max_acc_idx, 'file']
    print(f"Highest accuracy: {file_max_acc} ({df.loc[max_acc_idx, 'accuracy']})")
    # Lowest accuracy
    min_acc_idx = df['accuracy'].idxmin()
    file_min_acc = df.loc[min_acc_idx, 'file']
    print(f"Lowest accuracy: {file_min_acc} ({df.loc[min_acc_idx, 'accuracy']})")
    # Combined precision and accuracy
    df['prec_acc_sum'] = df['precision'] + df['accuracy']
    max_sum_idx = df['prec_acc_sum'].idxmax()
    file_max_sum = df.loc[max_sum_idx, 'file']
    print(f"Highest combined precision and accuracy: {file_max_sum} prec: ({df.loc[max_sum_idx, 'precision']}) acc: ({df.loc[max_sum_idx, 'accuracy']})")
    min_sum_idx = df['prec_acc_sum'].idxmin()
    file_min_sum = df.loc[min_sum_idx, 'file']
    print(f"Lowest combined precision and accuracy: {file_min_sum} prec: ({df.loc[min_sum_idx, 'precision']}) acc: ({df.loc[min_sum_idx, 'accuracy']})")
    ...
    ax9_pcr.set_title('PCR Metrics Distribution')
    ...
    ax9_pol.set_title('Polonies Metrics Distribution')

    # Shared legend on the second subplot
    handles = [Patch(facecolor=COLOR_MAP[m], label=m) for m in METRICS]
    ax9_pol.legend(handles=handles, title='Metric', loc='upper right')
